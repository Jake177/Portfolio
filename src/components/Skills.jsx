        ...
        <section id='skills' className='relative min-h-screen w-full gradient-bg grid-bg text-yellow-500 px-6 py-24 sm:py-28 isolate'>
            ...
            <div className='max-w-6xl mx-auto flex flex-col gap-10'>
                <header className='space-y-4 max-w-3xl fade-in-up'>
                    <span className='normal-font uppercase tracking-[0.45em] text-xs sm:text-sm text-yellow-500/70 inline-block px-4 py-2 rounded-full border border-yellow-500/20 bg-yellow-500/5'>Capabilities</span>
                    <h2 className='special-font text-4xl sm:text-5xl leading-tight text-gradient'>A toolkit anchored in cloud delivery, shipped with polish.</h2>
                    {/* 装饰分割线 */}
                    ...
                    <div className='overflow-x-auto snap-x snap-mandatory pb-6 -mx-6 px-6 sm:-mx-10 sm:px-10 no-scrollbar scroll-smooth' ref={scrollRef}>
                        <div className='flex gap-5 min-w-full'>
                            {skillCards.map((card, index) => (
                                <div
                                    key={card.title}
                                    className='skill-card snap-start shrink-0 w-[260px] sm:w-[300px] lg:w-[320px] rounded-3xl glass-card px-5 py-6'
                                >
